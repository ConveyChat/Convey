...
async function main() {
  ...
  const stream = await client.graphql(operation, message => {
    if (message.type === 'data') {
      console.log(message);
      const raw = decoder.decodeMessage(
        message.data.searchTransactions.node.matchingLogs[0].data
      );
      ...
      if (rawmsg[0] === 'mm') {
        ...
        if (
          ...
          console.log(
            `${web3.eth.defaultAccount} is not the intended recepient, ${msg.receiver} is`
          );
        }
      } else {
